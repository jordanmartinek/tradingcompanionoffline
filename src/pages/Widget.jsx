...
export default function Widget() {
  ...
  const [emaDirection, setEmaDirection] = useState(null); // 'above' | 'below' | null
  ...
  const handleExecute = async () => {
    ...
    // Create a trade record if there's an active session
    if (session) {
      ...
      await Trade.create({
        ...
        r_multiple: 0,
        pnl: 0,
        rule_compliance: entryRules.map(r => ({ rule: r.title, followed: r.enabled })),
      });
      setTrades(prev => [...prev, { slot_index: slotIndex }]);
    }

    // Reset all rules after execution
    await resetAllRules();
  ...
  return (
    <div className={cn(
      ...
      <div className="px-3 py-2.5 border-t border-zinc-800/50 bg-zinc-900/50">
        {showSuccess ? (
          <div className="w-full py-2.5 rounded-md bg-emerald-500/20 border border-emerald-500/40 text-center">
            <span className="text-emerald-300 text-sm font-medium">Trade Logged — Rules Reset</span>
          </div>
        ) : allSlotsFilled ? (
          ...
          <button
            ...
            className={cn(
              ...
              executed && 'opacity-50 cursor-not-allowed'
            )}
          >
            Execute Trade
          </button>
